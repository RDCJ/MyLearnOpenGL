...
vec3 ViewLightPos[LIGHT_MAX_NUM];
vec3 ViewLightDirection[LIGHT_MAX_NUM];
vec3 FragPos;
vec3 Normal;
vec3 diffuse_v;
vec3 specular_v;
...
vec3 CalcLight(int light_idx, vec3 normal, vec3 viewDir)
{
    ...
    // 计算从片段至光源的光线方向
    vec3 lightDir = float(light.type == 0) * (-ViewLightDirection[light_idx]) + 
                    float(light.type != 0) * (ViewLightPos[light_idx] - FragPos);
    ...
    if (light.type == 2)
    {
        ...
        float theta = dot(-lightDir, normalize(ViewLightDirection[light_idx]));
        float epsilon = light.innerCutOff - light.outerCutOff;
        spot_intensity = clamp((theta - light.outerCutOff) / epsilon, 0, 1);
    }

    // 计算光照衰减
    float distance = length(ViewLightPos[light_idx] - FragPos);
    float attenuation = 1.0 / (light.constant + light.linear * distance + light.quadratic * distance * distance);

    // 环境光照
    // 加上遮蔽因子
    vec3 ambient = light.ambient * diffuse_v * ambient_ssao;
...
void main()
{
    ...
    ambient_ssao = texture(ssao, TexCoord).r;

    for (int i=0; i<light_num; i++)
    {
        ViewLightPos[i] = vec3(view * vec4(lights[i].position, 1.0));
        ViewLightDirection[i] = vec3(view * vec4(lights[i].direction, 0.0));
    }

    // 观察方向(片段->摄像机)
    // viewpos 为 (0.0.0)，在观察空间中
