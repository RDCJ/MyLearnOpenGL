...
vec3 CalcLight(Light light, vec3 normal, vec3 viewDir)
{
    ...
    {
        lightDir = light.position - FragPos_;
    }
    ...
    // 环境光照
    vec3 ambient = light.ambient * vec3(texture(material.texture_diffuse0, TexCoord_));
    ...
    vec3 diffuse = diffuse_param * light.diffuse * vec3(texture(material.texture_diffuse0, TexCoord_));
    ...
    vec3 specular = specular_param * light.specular * vec3(texture(material.texture_specular0, TexCoord_));

    //vec3 emission = vec3(texture(material.emission, TexCoord_));
...
vec4 CalcEnvironmentReflection(vec3 normal, vec3 viewDir)
{
    ...
    // 用反射方向在立方体贴图上采样
    return texture(material.texture_ambient0, TexCoord_) * texture(cube_map, reflectDir);
