...
vec3 CalcLight(Light light, vec3 normal, vec3 viewDir, vec3 lightDir, vec2 texcoords)
{
    // 聚光光源计算强度
...
{   
    ...
    for (int i=0; i<light_num; i++)
        result += CalcLight(lights[i], TangentNorm, TangentViewDir, TangentLightDir[i], mapping_texcoords);
